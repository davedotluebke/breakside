...
| 2 | **Players appear only where an event places them.** A roster strip shows the whole line; a player steps onto the field the first time an event locates them and stays at their last spot. | Only the thrower and receiver (defender, puller) of each event are ever located. Anything else would be invented. |
...
| 7 | **Speed slider stops: Live · 1× · 2× · 4× · Play after play.** Play after play is the rightmost stop, not a separate toggle. Live is only offered while the game is in progress. | One control; no mode toggles fighting each other. |
