...
- **Entry point**: the one floating mic FAB, not a button of its own. `narration/micButton.js` holds a target per narration layer and picks between them at press time: `getActiveTab() === 'line'` → lineup, anything else in-game → event narration. A **non-idle layer always wins over the tab**, so switching tabs mid-session still offers "stop" for the work that's actually running rather than starting a second one. The same tap-toggle and hold-to-record gestures drive both. Phase colours are shared (micButton.css): lineup's `processing` reuses `.mic-finalizing`, so there is no fifth state. `applyTabState()` calls `window.narrationMicButton.refresh()` so the tooltip swaps with the tab instead of waiting on the FAB's 500ms visibility poll (background tabs throttle that hard). The Lines-tab mic button that shipped in build 1105 is gone; `lineupNarration.js` now owns only the inline status strip under the Select Line toolbar.
