...
**Decision (2024-12-07):** Migration tooling not needed. The only data worth preserving (4 Team D games) was already migrated using `scripts/migrate_games_to_ids.py`. 
...
- [x] All 4 Team D games have `teamId`, `rosterSnapshot`, and player ID references
...
- [x] Index test: rebuild index → verify queries return correct results (`test_existing_data.py` - 24 tests)
  ...
  - Team D specific tests (4 games, 18 players)
...
6. ✅ Existing data migrates cleanly *(4 BRK games migrated via Python script)*
