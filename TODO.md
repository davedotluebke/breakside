...
- [ ] **Code health: `ui/activePlayersDisplay.js`'s sticky active-players table is dead code** (found during the `teams/` refactor, D2). `updateActivePlayersList` / `createActivePlayersTable` / `makeColumnsSticky` target `#activePlayersTable` / `#tableContainer`, but neither element exists anywhere in `index.html` — the whole codepath is unreachable from any live screen. The actual in-game "before point" active-players table is `game/selectLine.js`'s panel-based system (`#panelActivePlayersTable`, `makePanelColumnsSticky()`), a near-duplicate implementation that's the one users actually see. Either delete the dead table code in `activePlayersDisplay.js`, or confirm there's a reason it's still there and wire it up. If deleting, also drop its now-unused sticky CSS (`.active-checkbox-column` etc. in `main.css`) — but check `teams/rosterManagement.js` first, which reuses `.active-checkbox-column` on the (very much alive) team roster table's checkbox cell.
