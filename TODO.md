...
- [ ] **Polish**: Stats-screen UI improvements (game summary + event roster)
  - **Explain column headers on long-press.** Several abbreviations (HA, Huck HA, Ds, +/-, ..per pt, Comp%, Huck%) are opaque to coaches who haven't used the app before. Long-press any column header to bring up a small modal/popover with the metric's name and a one-sentence definition.
  - **Rename `CSV` button → `Export to CSV`** and make it export only checked rows on the event-roster screen. Pickup rows have no checkbox today but are always attending — treat them as always-on (currently the CSV-export code at `teams/eventRoster.js:400 exportEventRosterCSV` writes "Yes" for pickups, keep that). Game-summary CSV has no checkboxes, so just rename the button there for consistency. Naturally defaults the event export to "players who actually played in this event."
  - **Wrap fix: split breaks and holds onto separate lines** in the team-stats summary. `formatTeamStatsLine()` currently joins them with `•`; on a narrow phone the bullet wraps mid-stat. Render two lines (or two `<div>`s) so each stat stays whole.
  - **Add team totals for points played** alongside the existing time-played total. The aggregate row already has the totals; `gameSummary.js:134` deliberately leaves the Pts cell blank ("not meaningful as team total" — but it actually is meaningful: it's player-points summed, i.e. line slots filled across the game). Show it.
  - **Inline the phase dropdown next to Review / trash on each event game row.** Currently the picker sits on its own row below ("Phase: [—]") which doubles the height of every event-game item. Move it into the same flex row as the Review button and trash icon, drop the "Phase:" label, and use "Select phase" as the empty-state option text in the `<select>` itself. Touch points: `renderGameItem` in `teams/teamSelection.js` (lines ~1679–1716 build the `phaseRow`); collapse into `line2` instead of appending a separate row, and update `.game-phase-row` / `.game-phase-select` CSS to suit.
  - **Auto-assign games to Day-N phases by start date.** Add a checkbox in the event settings dialog (the same dialog as the phases editor) labeled something like "Auto-label phases by day". When checked, the event settings save handler walks `event.gameIds`, groups their `gameStartTimestamp` by calendar day in local time, creates / reuses ordered phases named "Day 1", "Day 2", … in start-date order, and writes a `phase` to each game. First tap must show a confirm alert ("This will overwrite any existing phase labels on N games. Continue?") and only apply on confirm. Subsequent toggles can persist a `defaults.autoPhaseByDay: true` flag on the event so future games picked up by the event get auto-labeled at sync time (decide whether that's worth the complexity or whether the checkbox is purely a one-shot action). Touch points: `teams/teamSelection.js showEventSettingsDialog` (UI + save handler), `store/sync.js` (batch PATCH or a small server endpoint to bulk-update phases), and possibly `TournamentEvent.defaults` if the auto-mode is persistent.
  - **Report breaks against both denominators: D-points and D-possessions.** Today the team-stats line shows `Breaks: x/y D-points` where y is the number of points the team started on defense. But a defensive point can contain multiple defensive possessions if the team gets the disc back after a turnover, so "break rate per defensive opportunity" understates how often the team converts a stop. Add a parallel `Breaks: x/z D-possessions` figure where z = count of all defensive possessions across the game/event (any possession with `possession.offensive === false`). Both numbers are useful — point-rate is the headline tournament metric, possession-rate is the truer measure of D-line conversion efficiency. Extend `getGameTeamStats` / `getEventTeamStats` in `utils/eventStats.js` to compute both, and update `formatTeamStatsLine` to show them on separate lines (composes with the wrap-fix item above).
  - **Sticky leftmost two columns + sticky header row** on both stats tables (`gameSummaryRosterList` and `eventRosterList`). Checkbox column and player-name column should stay fixed while horizontally scrolling on a phone; the header row should stay fixed while vertically scrolling. CSS `position: sticky` on the relevant `<th>` / `<td>` cells with appropriate `left`/`top` offsets. Watch out for the `overflow` ancestor — `position: sticky` is silently disabled if any parent has `overflow: hidden` or `auto` that isn't the actual scroll container.
