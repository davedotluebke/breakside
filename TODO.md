...
- **Needs Dave — prod stats spot-check (G8).** Verifying Team A / Team B /
  Team C survived the name→ID stats migration needs a prod data read (SSH key auth);
...
- **G11.1–.2 exercised locally 2026-07-19** (two-tab multi-coach via
  ...
  2. 🔴 **Id-era `pendingNextLine` soft-locks Start Point.** On the real Team-E game the
     stored line holds player IDs; the Line panel matches checkboxes by NAME → nothing
     renders checked, the Next Line header shows raw ids ("Dana-mixr, …"), and
  ...
  4. 🟠 **Zombie point in real data:** Team-E's stored point 13 has `winner` set +
