...
from inside ``breakside_server/``, which puts the package dir on sys.path) and
as a package (``from breakside_server.main import app``). main.py used to
...
def import_server_module(name: str):
    ...
    package-qualified form (``breakside_server.config``), whichever matches
