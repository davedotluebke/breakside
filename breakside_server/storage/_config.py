...
from inside ``breakside_server/``, which puts the package dir on sys.path so
``import config`` works) and as a package (``from breakside_server.main import
...
try:
    ...
    config = importlib.import_module("breakside_server.config")
