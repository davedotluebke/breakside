...
Run with: cd breakside_server && python -m pytest test_admin_group_games.py -v
"""
import json
import os
import subprocess
...
def data(tmp_path):
    d = tmp_path / "data"
    d.mkdir()
    make_team(d, "Team-B-abcd", "Team B",
              player_ids=["Alice-1111", "Bob-2222"])
    make_team(d, "Offline-Test-9999", "Offline Test")  # no team playerIds
    make_game(d, "g1", "Team-B-abcd", "Sharks", "2026-06-19T18:00:00.000Z")
    make_game(d, "g2", "Team-B-abcd", "Bears", "2026-06-19T20:00:00.000Z")
...
def test_teams_and_games_listing(data):
    ...
    assert "Team B" in r.stdout and "Offline Test" in r.stdout

    r = run(data, "games", "--team", "Team B")
    assert r.returncode == 0, r.stderr
...
def test_group_creates_event_and_links(data):
    ...
    assert set(idx["teamGames"]["Team-B-abcd"]) == {"g1", "g2"}
...
def test_conflict_requires_force(data):
    run(data, "group", "--team", "Team B", "--event", "First",
        "--games", "g1", "--yes")
    eid = only_event(data)["id"]
...
def test_unlink_and_delete(data):
    run(data, "group", "--team", "Team B", "--event", "SL",
        "--games", "g1", "g2", "--yes")
    eid = only_event(data)["id"]
