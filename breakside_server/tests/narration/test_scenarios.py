...
Skipping behavior:
  - These scenarios stream audio through the OpenAI Realtime API and run
    ...
    ``pytest breakside_server/`` run stays fast and deterministic even on
  ...
  - All tests carry the ``live_llm`` marker (registered in
    breakside_server/conftest.py), so ``-m "not live_llm"`` /
    ``-m live_llm`` can also deselect/select them.

Run with:
  NARRATION_LIVE_TESTS=1 pytest breakside_server/tests/narration/            # all scenarios
  NARRATION_LIVE_TESTS=1 pytest breakside_server/tests/narration/ -k single_throw  # one
  NARRATION_LIVE_TESTS=1 pytest breakside_server/tests/narration/ -s         # show metrics
"""
