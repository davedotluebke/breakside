...
Designed to be runnable two ways:
  - As a CLI:  python -m breakside_server.tests.narration.runner SCENARIO_DIR
...
def main(argv: List[str]) -> int:
    if len(argv) < 2:
        print("usage: python -m breakside_server.tests.narration.runner SCENARIO_DIR", file=sys.stderr)
        return 2
    sd = Path(argv[1])
    if not sd.is_dir():
