...
```bash
pip install -r breakside_server/requirements.txt   # picks up websockets + soundfile
...
```bash
python -m breakside_server.tests.narration.tools.generate_synthetic_audio \
    breakside_server/tests/narration/scenarios/001_single_throw
```
...
```bash
python -m breakside_server.tests.narration.runner \
    breakside_server/tests/narration/scenarios/001_single_throw
```
...
```bash
NARRATION_LIVE_TESTS=1 pytest breakside_server/tests/narration/ -s
```

These scenarios hit live paid APIs and are non-deterministic, so they're **opt-in**: without `NARRATION_LIVE_TESTS=1` they all skip (they also carry a `live_llm` pytest marker). That keeps the default `pytest breakside_server/` run fast and deterministic even on machines with API keys exported.
...
3. Write `roster.json` — at minimum:
   ...
   ```
4. Write `expected.json` — a JSON array of expected events. Each event has a `kind` (`throw` / `turnover` / `defense` / `opponent_score`) plus the relevant fields. Mirror the schema documented in `breakside_server/narration.py`.
